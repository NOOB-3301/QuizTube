  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-800 to-blue-900 text-white p-4">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-2xl md:text-3xl font-semibold text-center mb-8">{data.videoTitle}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          ...
          <div className="bg-white text-black rounded-xl shadow-md p-6 max-h-[800px] overflow-y-auto">
                    ...
                    <div className="mt-2 space-y-2">
                      {q.options?.map((option, optionIndex) => (
                        <div key={optionIndex} className="flex items-center space-x-2">
                          ...
                          />
                          <label htmlFor={`q${qIndex}-opt${optionIndex}`} className="text-sm">
                            {option}
                          </label>
                        </div>
                      ))}
            ...
            {data.type === "long-answer" && data.questions && (
              <div>
                <h3 className="text-xl font-semibold mb-4">Long Answer Questions</h3>
                {data.questions.map((q, qIndex) => (
                  <div key={qIndex} className="mb-6">
                    <p className="font-medium">{q.question}</p>
